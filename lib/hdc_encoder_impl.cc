...
hdc_encoder::sptr hdc_encoder::make(int channels, int bitrate, bool ps_enable)
{
    return gnuradio::get_initial_sptr(new hdc_encoder_impl(channels, bitrate, ps_enable));
}
 ...
 */
hdc_encoder_impl::hdc_encoder_impl(int channels, int bitrate, bool ps_enable)
                ...
                gr::io_signature::make(1, 1, sizeof(unsigned char)))
{
    ...
    /* PS requires stereo input but encodes to mono internally */
    if (ps_enable && channels != 2) {
        throw std::runtime_error("hdc_encoder: Parametric Stereo requires 2 channels");
    }
    ...
    if (aacEncOpen(&handle, 0, channels) != AACENC_OK) {
        throw std::runtime_error("hdc_encoder: Unable to open encoder");
    }

    /* Select AOT: 127 = HDC, 128 = HDC with Parametric Stereo */
    int aot = ps_enable ? 128 : 127;  /* AOT_HDC_PS : AOT_HDC */
